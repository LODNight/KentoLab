...
// Sidebar DOM refs - lazy (resolved after loadSharedComponents injects the sidebar)
...
// Called after loadSharedComponents() has injected sidebar HTML
function initSidebarEvents() {
  const mainSidebar = getMainSidebar();
  const sidebarToggleDesktop = getSidebarToggleDesktop();
  const sidebarToggleIcon = getSidebarToggleIcon();
  const sidebarToggleMobile = getSidebarToggleMobile();
  const sidebarNavContainer = getSidebarNavContainer();
  const algoSearchInput = getAlgoSearchInput();
...
function selectMegaGroup(groupId) {
  ...
  if (document.getElementById("workspace-title")) document.getElementById("workspace-title").innerHTML = `<i class="fa-solid fa-graduation-cap text-indigo-500 mr-1"></i> Không Gian Học Tập`;
  if (document.getElementById("workspace-subtitle")) document.getElementById("workspace-subtitle").innerText = "Lựa chọn một chuyên đề học thuật bên dưới để khám phá phòng thí nghiệm trực quan sinh động.";
  if (document.getElementById("algo-category-badge")) document.getElementById("algo-category-badge").innerText = "TỔNG QUAN";
}
...
function selectAlgorithm(algoId, catId) {
  ...
  if (document.getElementById("algo-category-badge")) document.getElementById("algo-category-badge").innerText = algoCategories.find(c => c.id === catId).name;
  if (document.getElementById("workspace-title")) document.getElementById("workspace-title").innerHTML = `<i class="fa-solid fa-cube text-indigo-500 animate-pulse"></i> ${algoId.toUpperCase().replace("_", " ")}`;
  if (document.getElementById("workspace-subtitle")) document.getElementById("workspace-subtitle").innerText = profile.shortSummary;
  ...
  const lockOverlay = document.getElementById("academy-lock-overlay");
  if (isUnlocked) {
...
function loadDynamicSandboxParameters(profile) {
  ...
  if (document.getElementById("lbl-complexity-time")) document.getElementById("lbl-complexity-time").innerText = profile.best;
  if (document.getElementById("lbl-complexity-space")) document.getElementById("lbl-complexity-space").innerText = profile.space;
  if (document.getElementById("algo-short-summary")) document.getElementById("algo-short-summary").innerText = profile.shortSummary;
}

// Populate academic dictionary sheet (Theory View)
function populateAcademicTheoryTab(profile, algoId) {
  if (document.getElementById("theory-title")) document.getElementById("theory-title").innerHTML = `<i class="fa-solid fa-graduation-cap text-indigo-500"></i> Sổ Tay Học Thuật: ${algoId.toUpperCase().replace("_", " ")}`;
  if (document.getElementById("theory-subtitle")) document.getElementById("theory-subtitle").innerText = `Đầy đủ 10 tiêu chí học thuật tiêu chuẩn của giải thuật này.`;
...
// Called after loadSharedComponents + DOM is ready, on algorithm.html only
function initWorkspaceEvents() {
  const modal = document.getElementById("custom-input-modal");

  // Playback controls
  const _play = document.getElementById("btn-playback-play");
  const _next = document.getElementById("btn-playback-next");
  const _prev = document.getElementById("btn-playback-prev");
  const _first = document.getElementById("btn-playback-first");
  const _last = document.getElementById("btn-playback-last");
  const _slider = document.getElementById("slider-step-timeline");

  if (_play) _play.addEventListener("click", togglePlayback);
  if (_next) _next.addEventListener("click", () => { pausePlayback(); nextStep(); });
  if (_prev) _prev.addEventListener("click", () => { pausePlayback(); prevStep(); });
  if (_first) _first.addEventListener("click", () => { pausePlayback(); currentStepIdx = 0; updateSandboxUI(); });
  if (_last) _last.addEventListener("click", () => { pausePlayback(); currentStepIdx = steps.length - 1; updateSandboxUI(); });
  if (_slider) _slider.addEventListener("input", (e) => { pausePlayback(); currentStepIdx = parseInt(e.target.value); updateSandboxUI(); });

  // Custom array input modal
  const btnCancel = document.getElementById("btn-modal-cancel");
  const btnSubmit = document.getElementById("btn-modal-submit");
  if (btnCancel) btnCancel.addEventListener("click", () => modal && modal.classList.add("hidden"));
  ...
  // SVG Graph canvas click listener
  const _svgGraph = getSvgGraph();
  if (_svgGraph) _svgGraph.addEventListener("click", function (e) {
    if (!isAddingNodeMode) return;

    const rect = _svgGraph.getBoundingClientRect();
    ...
    if (addBtn) addBtn.classList.remove("bg-indigo-600");
  });
...
function handleAlgoSearch(e) {
  ...
  if (document.getElementById("workspace-title")) document.getElementById("workspace-title").innerHTML = `<i class="fa-solid fa-magnifying-glass text-indigo-500"></i> Kết quả tìm kiếm`;
  if (document.getElementById("workspace-subtitle")) document.getElementById("workspace-subtitle").innerText = `Tìm kiếm tất cả giải thuật khớp với từ khóa: "${query}"`;
  if (document.getElementById("algo-category-badge")) document.getElementById("algo-category-badge").innerText = "TÌM KIẾM";
...
function copyToClipboard(id) {
  ...
  // =========================================================================
  // MPA MULTI-PAGE APPLICATION ROUTING & VIEW CONFIGURATION
  // =========================================================================
  ...
  function initDashboard() {
    ...
    const mega = params.get('mega') || 'core';
    selectMegaGroup(mega, true);
  }
  ...
  function initAlgorithm() {
    ...
    const params = new URLSearchParams(window.location.search);
    const algoId = params.get('algo') || 'binary';
    const catId = params.get('cat') || 'searching';

    activeAlgoId = algoId;
    activeCategoryId = catId;

    selectAlgorithm(algoId, catId, true);
  }
  ...
  // Override original selectAlgorithm
  const originalSelectAlgorithm = selectAlgorithm;
  selectAlgorithm = function (algoId, catId, bypassRedirect = false) {
    if (!bypassRedirect && !window.location.pathname.endsWith('algorithm.html')) {
      window.location.href = 'algorithm.html?algo=' + algoId + '&cat=' + catId;
      return;
    }
    originalSelectAlgorithm(algoId, catId);
  };

  // Override goBackToDashboard
  const originalGoBackToDashboard = goBackToDashboard;
  goBackToDashboard = function () {
    if (!window.location.pathname.endsWith('index.html') && !window.location.pathname.endsWith('/')) {
      window.location.href = 'index.html?mega=' + activeMegaGroupId;
      return;
    }
    originalGoBackToDashboard();
  };

  // Override selectMegaGroup
  const originalSelectMegaGroup = selectMegaGroup;
  selectMegaGroup = function (groupId, bypassRedirect = false) {
    if (!bypassRedirect && !window.location.pathname.endsWith('index.html') && !window.location.pathname.endsWith('/')) {
      window.location.href = 'index.html?mega=' + groupId;
      return;
    }
    originalSelectMegaGroup(groupId);
  };
